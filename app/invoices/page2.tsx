...
export default function InvoicesPage() {
  ...
  const handleUploadToGoogleDrive = async (invoice: any) => {
    ...
    try {
      ...
      setInvoices(
        invoices.map((i) =>
          i.id === invoice.id ? { ...i, drivePdfName: displayName, drivePdfFileId: fileId, updatedAt } : i,
        ),
      )
      toast({
              ...
              <TableBody>
                {filteredInvoices.map((invoice) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell>{formatDocumentDateBerlin(invoice.invoiceDate, "MMM d, yyyy")}</TableCell>
                    <TableCell>{formatDocumentDateBerlin(invoice.dueDate, "MMM d, yyyy")}</TableCell>
